...
if __name__ == '__main__':
    ...
    # 弹窗
    window = tkinter.Tk()
    window.title("文章网络图片下载       Authored By: Chen Jie")
    window.geometry("600x200")

    scroll = tkinter.Scrollbar(window, orient=tkinter.VERTICAL)
    scroll.pack(side=tkinter.RIGHT, fill=tkinter.Y)


    def selectDir():
        dirPath = filedialog.askdirectory(initialdir=path.dirname(__file__))
        print(dirPath)
        print(get_files_list(dirPath))
        for index, file in enumerate(get_files_list(dirPath)):
            print(index)
            print(file)
            # 复选框
            chk_state = tkinter.BooleanVar()
            chk_state.set(True)
            chk = tkinter.Checkbutton(window, text=file[len(dirPath) + 1:], var=chk_state)
            chk.pack(side=tkinter.LEFT)


    button = tkinter.Button(window, text="选择文件夹", command=selectDir)
