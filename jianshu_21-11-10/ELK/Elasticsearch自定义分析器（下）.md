...
- 1）**TF（Term Frequency）**：词频，即单词在文档中出现的次数，词频越高，相关度越高。TF 的计算永远是100%的精确，这是因为它是一个文档级的计算，文档内容可以在本地分片中获取。公式为`tf(t in d) = √frequency`，即term在文件 d 的词频（tf）是这个术语在文档中出现次数的平方根)![image.png](Elasticsearch自定义分析器（下）.assets\8444372a802e40cb989cd44666aeb58d.png)
- 2）**IDF（Inverse Document Frequency）**：逆向文档词频 ，计算公式为 term/document ，即单词出现的文档数越少，相关度越高。公式为`idf(t) = 1 + log ( numDocs / (docFreq + 1)) `，即术语t的逆向文档频率（Inverse document frequency）是：索引中文档数量除以所有包含该术语文档数量后的对数值。![image.png](Elasticsearch自定义分析器（下）.assets\4423b662d42a4c14b9ab60328de99c40.png)
...
4.x之前的计算模型如下
![image.png](Elasticsearch自定义分析器（下）.assets\cbd6b7e348fb4c429807ac4763d4e756.png)
...
衰减函数（Decay Function）提供了一个更为复杂的公式，它描述了这样一种情况：对于一个字段，它有一个理想的值，而字段实际的值越偏离这个理想值（无论是增大还是减小），就越不符合期望。 有三种衰减函数——线性（linear）、指数（exp）和高斯（gauss）函数，它们可以操作数值、时间以及 经纬度地理坐标点这样的字段。常见的Decay function有以下三种：
  ...
  ![20191221164124868.png](Elasticsearch自定义分析器（下）.assets\0f5c2793983545f194905f3d46dc8620.png)

- **gauss**

  正常衰减，计算如下：

   ![image](Elasticsearch自定义分析器（下）.assets\7ba8e841c02f4f438568756e83afff13.png)
...
- **exp**
  ...
  ![image](Elasticsearch自定义分析器（下）.assets\4a94d584127445d1b7bf506f834b371a.png)
...
- **linear**
  ...
  线性衰减，计算如下：
   ...
   ![image](Elasticsearch自定义分析器（下）.assets\5bd11b69cdc44fed8f33de48bf2d7e64.png)

  其中参数`s`被再次计算，以确保该得分取值`decay`在距离`scale`从`origin`+ -`offset`

   ![image](Elasticsearch自定义分析器（下）.assets\99678048211b46e397da878d914d3e9b.png)
