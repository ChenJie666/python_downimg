...
threadpool是一个比较老的模块了，主键被其他模块取代
```
import threadpool
import time
...
if __name__ == '__main__':
    ...
    end1 = time.time()
    print("time2: " + str(end1 - start1))
    # 方式二
    start2 = time.time()
    with ThreadPoolExecutor(3) as executor1:
        executor1.map(sayhello, seed)
    end2 = time.time()
    print("time3: " + str(end2 - start2))
```
...
```
这里多线程耗时是单线程的10倍，原因就是GIL锁导致的即是多线程也只有一个核在进行运算，线程间的切换导致了耗时增加，起到了反作用。
也印证了`CPU密集型的任务不能用多线程执行，而应该用多进程执行。`
